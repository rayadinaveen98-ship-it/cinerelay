...
if (!supabaseUrl || !serviceRoleKey || !internalSecret) {
  throw new Error('Missing evidence-summary-worker environment');
}
...
Deno.serve(async (request) => {
  try {
    ...
    const { data, error } = await supabase.rpc('refresh_event_summaries', {
      p_limit: limit,
    });
    ...
    console.error('evidence-summary-worker failure', error);
