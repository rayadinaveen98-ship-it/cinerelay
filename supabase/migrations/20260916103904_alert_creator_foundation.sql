...
begin
  ...
  if p_start < p_end then
    ...
    if v_in_quiet then v_end_local := v_local::date + p_end; end if;
  else
    v_in_quiet := v_local_time >= p_start or v_local_time < p_end;
    if v_in_quiet then
      if v_local_time >= p_start then v_end_local := (v_local::date + 1) + p_end;
      else v_end_local := v_local::date + p_end; end if;
    end if;
  end if;
...
begin
  ...
  with eligible as (
    select f.user_id, coalesce(p.alert_mode, 'INSTANT') as alert_mode,
    ...
    left join public.user_alert_event_preferences ep on ep.user_id = f.user_id and ep.event_type = v_event.event_type
    where f.entity_id = v_event.primary_entity_id and f.active = true and f.created_at <= v_event.created_at
  ), push_rows as (
    select e.*, public.next_alert_allowed_at(v_now, e.timezone_name, e.quiet_hours_enabled,
      e.quiet_start_local, e.quiet_end_local, e.critical_bypass_quiet_hours, v_event.priority_band) as scheduled_for
    from eligible e
    where e.event_enabled = true and e.effective_mode in ('INSTANT','BOTH')
  ...
  insert into public.alert_deliveries (user_id, event_id, delivery_kind, dedupe_key, status, scheduled_for, payload)
  select r.user_id, v_event.id, 'PUSH', 'alert:' || r.user_id::text || ':' || v_event.id::text || ':push',
    case when r.scheduled_for > v_now + interval '1 second' then 'DEFERRED' else 'PENDING' end, r.scheduled_for,
    jsonb_build_object('eventId', v_event.id,'entityId',v_event.primary_entity_id,'eventType',v_event.event_type,
      'verificationState',v_event.verification_state,'priorityBand',v_event.priority_band,'headline',v_event.headline)
  from push_rows r on conflict (dedupe_key) do nothing;
  ...
  with eligible as (
    ...
    select e.*, public.next_alert_allowed_at(
      public.next_alert_digest_at(v_now, e.timezone_name, e.digest_hour_local), e.timezone_name,
      e.quiet_hours_enabled, e.quiet_start_local, e.quiet_end_local,
      e.critical_bypass_quiet_hours, v_event.priority_band) as scheduled_for
    from eligible e
    where e.event_enabled = true and e.effective_mode in ('DIGEST','BOTH')
      and public.alert_priority_rank(v_event.priority_band) <= public.alert_priority_rank(e.minimum_priority_band)
      and (v_event.verification_state <> 'RUMOR' or e.include_rumors)
      and (v_event.verification_state <> 'DEVELOPING' or e.include_developing)
  )
  insert into public.alert_deliveries (user_id,event_id,delivery_kind,dedupe_key,status,scheduled_for,payload)
  select r.user_id,v_event.id,'DIGEST','alert:'||r.user_id::text||':'||v_event.id::text||':digest','DEFERRED',r.scheduled_for,
    jsonb_build_object('eventId',v_event.id,'entityId',v_event.primary_entity_id,'eventType',v_event.event_type,
      'verificationState',v_event.verification_state,'priorityBand',v_event.priority_band,'headline',v_event.headline)
  from digest_rows r on conflict (dedupe_key) do nothing;
...
create or replace function public.upsert_canonical_event_with_evidence(
  p_event_id uuid,p_primary_entity_id uuid,p_event_type text,p_verification_state text,p_priority_band text,
  p_headline text,p_structured_data jsonb,p_dedupe_key text,p_classifier_version text,p_raw_item_id uuid
)
returns uuid language plpgsql set search_path = pg_catalog, public, extensions as $$
declare result_id uuid; had_evidence boolean;
begin
  ...
  insert into public.events (id,primary_entity_id,event_type,event_schema_version,detected_at,verification_state,priority_band,headline,structured_data,dedupe_key,status,classifier_version)
  values (p_event_id,p_primary_entity_id,p_event_type,1,now(),p_verification_state,p_priority_band,p_headline,coalesce(p_structured_data,'{}'::jsonb),p_dedupe_key,'ACTIVE',p_classifier_version)
  on conflict (dedupe_key) do update set
    verification_state=case when public.verification_rank(excluded.verification_state)<public.verification_rank(public.events.verification_state) then excluded.verification_state else public.events.verification_state end,
    priority_band=case when excluded.priority_band='CRITICAL' then 'CRITICAL' when public.events.priority_band='CRITICAL' then public.events.priority_band when excluded.priority_band='HIGH' then 'HIGH' when public.events.priority_band='HIGH' then public.events.priority_band when excluded.priority_band='NORMAL' then 'NORMAL' else public.events.priority_band end,
    headline=excluded.headline, structured_data=excluded.structured_data, classifier_version=excluded.classifier_version, updated_at=now()
  returning id into result_id;
  select exists(select 1 from public.event_evidence where event_id=result_id) into had_evidence;
  insert into public.event_evidence(event_id,raw_item_id,evidence_role,weight) values(result_id,p_raw_item_id,case when had_evidence then 'REPEAT' else 'PRIMARY' end,1) on conflict(event_id,raw_item_id) do nothing;
  perform public.plan_event_alerts(result_id); return result_id;
end;
$$;
...
create policy user_entity_follows_select_own on public.user_entity_follows for select to authenticated using ((select auth.uid())=user_id);
create policy user_entity_follows_insert_own on public.user_entity_follows for insert to authenticated with check ((select auth.uid())=user_id);
create policy user_entity_follows_update_own on public.user_entity_follows for update to authenticated using ((select auth.uid())=user_id) with check ((select auth.uid())=user_id);
create policy user_entity_follows_delete_own on public.user_entity_follows for delete to authenticated using ((select auth.uid())=user_id);
create policy user_alert_preferences_select_own on public.user_alert_preferences for select to authenticated using ((select auth.uid())=user_id);
create policy user_alert_preferences_insert_own on public.user_alert_preferences for insert to authenticated with check ((select auth.uid())=user_id);
create policy user_alert_preferences_update_own on public.user_alert_preferences for update to authenticated using ((select auth.uid())=user_id) with check ((select auth.uid())=user_id);
create policy user_alert_preferences_delete_own on public.user_alert_preferences for delete to authenticated using ((select auth.uid())=user_id);
create policy user_alert_event_preferences_select_own on public.user_alert_event_preferences for select to authenticated using ((select auth.uid())=user_id);
create policy user_alert_event_preferences_insert_own on public.user_alert_event_preferences for insert to authenticated with check ((select auth.uid())=user_id);
create policy user_alert_event_preferences_update_own on public.user_alert_event_preferences for update to authenticated using ((select auth.uid())=user_id) with check ((select auth.uid())=user_id);
create policy user_alert_event_preferences_delete_own on public.user_alert_event_preferences for delete to authenticated using ((select auth.uid())=user_id);
create policy alert_deliveries_select_own on public.alert_deliveries for select to authenticated using ((select auth.uid())=user_id);
...
comment on table public.user_entity_follows is 'Per-user entity follow graph. Alerts are planned only for active follows that existed when a canonical event was first created.';
comment on table public.user_alert_preferences is 'Per-user alert defaults for priority, verification, quiet hours and digest scheduling. Defaults are high-priority instant alerts with rumors disabled.';
comment on table public.user_alert_event_preferences is 'Optional per-event-type overrides layered on top of the user alert defaults.';
comment on table public.alert_deliveries is 'Idempotent alert outbox. P5 delivery workers consume due rows; event processing never sends directly to FCM.';
comment on function public.plan_event_alerts(uuid) is 'Plans deduplicated PUSH/DIGEST outbox rows from a canonical event and the follow/preference graph. No external delivery occurs here.';
