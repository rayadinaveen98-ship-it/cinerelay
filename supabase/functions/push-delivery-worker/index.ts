...
if (!supabaseUrl || !serviceRoleKey || !internalSecret) {
  throw new Error('Missing push-delivery-worker environment');
}
...
function messageData(target: LeasedTarget): Record<string, string> {
  ...
  const pairs: Array<[string, unknown]> = [
    ['alertDeliveryId', target.alert_delivery_id],
...
async function complete(
  ...
  const { error } = await supabase.rpc('complete_push_delivery_target', {
...
async function sendOne(account: ServiceAccount, accessToken: string, target: LeasedTarget): Promise<string> {
  ...
  // INVALID_ARGUMENT can represent a malformed payload as well as bad target data.
  // Do not deactivate a device unless FCM specifically reports UNREGISTERED.
...
Deno.serve(async (request) => {
  try {
    ...
    const requested = typeof body.limit === 'number' && Number.isFinite(body.limit) ? Math.floor(body.limit) : 25;
    const limit = Math.max(1, Math.min(requested, 100));

    // Validate credentials and mint authorization before leasing durable work so a
    // provider-auth outage cannot strand rows behind a fresh lease.
    const account = parseServiceAccount();
    const accessToken = await mintAccessToken(account);

    const { data: materialized, error: materializeError } = await supabase.rpc('materialize_due_push_targets', {
      p_limit: Math.max(limit, 100),
    });
    if (materializeError) throw materializeError;

    const { data: leased, error: leaseError } = await supabase.rpc('lease_push_delivery_targets', {
      p_limit: limit,
      p_lease_seconds: 60,
    });
    ...
    for (const target of targets) {
      try {
        ...
        // Network/timeouts can happen before an FCM response. Convert them into a
        // durable transient failure rather than waiting for lease expiry alone.
        console.error('push target transport failure', {
        ...
        try {
          ...
          // If completion itself fails the lease expiry remains the recovery path.
          console.error('push target completion failure', {
            targetId: target.target_id,
    ...
    console.error('push-delivery-worker failure', { message });
