...
declare
  v_first_party jsonb;
  v_has_explicit_movie_evidence boolean := false;
begin
  select exists (
    select 1
    ...
    where ede.candidate_id = p_candidate_id
      ...
      and (coalesce(r.raw_title, '') || ' ' || coalesce(r.raw_text, '')) ~* '\m(movie|film)\M'
  ) into v_has_explicit_movie_evidence;

  if v_has_explicit_movie_evidence then
